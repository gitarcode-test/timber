...
class WrongTimberUsageDetector : Detector(), UastScanner {
        ...
        for (i in types.indices) {
            ...
            valid =
                when (last) {
                    ...
                    'h',
                    'H' ->
                        type != java.lang.Boolean.TYPE && !Number::class.java.isAssignableFrom(type)
                    's',
                    'S' -> true
                    else -> true
                }
    ...
    private fun checkExceptionLogging(context: JavaContext, call: UCallExpression) {
        ...
        if (numArguments > 1 && isSubclassOf(context, arguments[0], Throwable::class.java)) {
            ...
            numArguments == 1 && !isSubclassOf(context, arguments[0], Throwable::class.java)
        ) {
    ...
    private fun checkElement(
        ...
        if (element is UBinaryExpression) {
            ...
            if (
                operator === UastBinaryOperator.PLUS || operator === UastBinaryOperator.PLUS_ASSIGN
            ) {
                val argumentType = getType(element)
                if (argumentType == String::class.java) {
                    if (
                        element.leftOperand.isInjectionHost() &&
        ...
        when (arguments.size) {
            ...
            3 -> {
                ...
                throw IllegalStateException(
                    "android.util.Log overloads should have 2 or 3 arguments"
                )
            }
        }
    ...
    companion object {
        ...
        val ISSUE_LOG =
            Issue.create(
                ...
                severity = WARNING,
                implementation =
                    Implementation(WrongTimberUsageDetector::class.java, JAVA_FILE_SCOPE)
            )
        val ISSUE_FORMAT =
            Issue.create(
